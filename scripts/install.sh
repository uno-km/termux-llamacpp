...
MODELS_DIR="$XDG_CACHE_HOME/termux-llamacpp/models"
ARCH="$(uname -m)"
...
printf '  Binary Target: %s/bin\n' "$PREFIX"
printf '  Library Target: %s/lib\n' "$PREFIX"
...
mkdir -p "$PREFIX/bin" "$PREFIX/lib" "$PREFIX/share/termux-llamacpp" "$MODELS_DIR"
...
if [ "$FROM_SOURCE" = "1" ]; then
    ...
    find build -name "*.so*" -type f -exec cp -a {} "$PREFIX/lib/" \; 2>/dev/null || true
    cp build/bin/llama-server "$PREFIX/bin/"
    cp build/bin/llama-cli "$PREFIX/bin/"
    chmod 0755 "$PREFIX/bin/llama-server" "$PREFIX/bin/llama-cli"
    ...
    for bin_name in llama-server llama-cli; do
        bin_sha="$(sha256sum "$PREFIX/bin/$bin_name" | awk '{print $1}')"
        cat <<EOF > "$PREFIX/share/termux-llamacpp/${bin_name}.build-receipt.json"
{
...
if [ "$FROM_SOURCE" != "1" ]; then
    ...
    if [ "$DOWNLOAD_SUCCESS" = "1" ]; then
        ...
        STAGING="$XDG_CACHE_HOME/termux-llamacpp/.staging-$VERSION"
        rm -rf "$STAGING"
        mkdir -p "$STAGING" "$MODELS_DIR"
        tar -xzf "$TMP/$ASSET" -C "$STAGING"

        # Install binaries directly to $PREFIX/bin
        find "$STAGING" -type f -name "llama-cli" -exec cp -f {} "$PREFIX/bin/" \;
        find "$STAGING" -type f -name "llama-server" -exec cp -f {} "$PREFIX/bin/" \;
        chmod 0755 "$PREFIX/bin/llama-cli" "$PREFIX/bin/llama-server" 2>/dev/null || true

        # Install shared libraries directly to $PREFIX/lib
        ...
        rm -rf "$STAGING"
        printf '  [termux-llamacpp] Prebuilt binary bundle successfully installed into %s/bin and %s/lib.\n' "$PREFIX" "$PREFIX"
    else
...
# ==============================================================================
# Step 3: Python & Node.js Ecosystem Package Auto-Provisioning (pip & npm)
# ==============================================================================
